      ...
      <div className="container mx-auto px-4 sm:px-6 max-w-7xl">
        ...
        <div className="flex items-center gap-2 mb-6">
          <Badge variant="secondary" className="text-muted-foreground">
            {filtered.length}{" "}
            {filtered.length === 1 ? "продукт" : filtered.length < 5 ? "продукта" : "продуктов"}
          </Badge>
        </div>

        {filtered.length > 0 ? (
          <AnimatePresence mode="popLayout">
            <motion.div layout className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-5">
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-muted-foreground text-sm mt-1">Попробуйте изменить фильтры поиска</p>
          </div>
        )}
      </div>
    </div>
